...
public class BdvPanel extends JPanel
{
	...
	public < T extends NumericType< T > > BdvPanel(
			...
			final int width, final int height )
	{
		...
		{
			if ( dC == -1 && dT == -1 )
					throw new IllegalArgumentException( "Four dimensional input image without CHANNEL axis must contain a TIME axis! Neither of both found..." );
		...
		final VoxelDimensions voxelDimensions = new FinalVoxelDimensions(
				imgPlus.axis( dX ).unit(),
				imgPlus.averageScale( dX ),
				imgPlus.averageScale( dY ),
				imgPlus.averageScale( dZ ) );
...
//			setPreferredSize( new Dimension( width, height ) );
		...
		brightnessPanel.setBorder( BorderFactory.createCompoundBorder(
	...
	private static < T > void initSetups(
			final ImgPlus< T > imgPlus,
			final int dC,
			final int dT,
			final int numSetups,
			...
			final List< SourceAndConverter< ? > > sources )
	{
		...
		if ( RealType.class.isInstance( type ) )
			initSetupsRealType( (ImgPlus) imgPlus, dC, dT, numSetups, voxelDimensions, sourceTransform, converterSetups, sources );
		else if ( ARGBType.class.isInstance( type ) )
			initSetupsARGBType( (ImgPlus) imgPlus, dC, dT, numSetups, voxelDimensions, sourceTransform, converterSetups, sources );
		else
			throw new IllegalArgumentException( "Input image must be of RealType or ARGBType! (input image type is " + type.getClass() + ")" );
	}

	private static < T extends RealType< T > > void initSetupsRealType(
			final ImgPlus< T > imgPlus,
			final int dC,
			final int dT,
			final int numSetups,
			...
			final List< SourceAndConverter< ? > > sources )
	{
		...
		for ( int setup = 0; setup < numSetups; ++setup )
		{
			final ImgPlusSource< T > source = ( dC == -1 ) ?
					...
					new ImgPlusSource< T >( Views.hyperSlice( imgPlus, dC, setup ), dT > dC ? dT - 1 : dT , setup, voxelDimensions, sourceTransform );
	...
	private static void initSetupsARGBType(
			final ImgPlus< ARGBType > imgPlus,
			final int dC,
			final int dT,
			final int numSetups,
			final VoxelDimensions voxelDimensions,
			...
			final List< SourceAndConverter< ? > > sources )
	{
		...
		for ( int setup = 0; setup < numSetups; ++setup )
		{
			final ImgPlusSource< ARGBType > source = ( dC == -1 ) ?
					...
					new ImgPlusSource< ARGBType >( Views.hyperSlice( imgPlus, dC, setup ), dT > dC ? dT - 1 : dT , setup, voxelDimensions, sourceTransform );
